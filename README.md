...
GitHub Pagesで公開する設定を追加しています。PRが `main` または `master` ブランチへマージされ、GitHub Actions の **Deploy static UI to GitHub Pages** が完了すると、以下のURLでUIを確認できます。
...
- `.github/workflows/pages.yml`: `main` / `master` への push または手動実行で、リポジトリ直下の静的UIをGitHub Pagesへデプロイします。
...
1. このPRを `main` または `master` ブランチへマージします。
